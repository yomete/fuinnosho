...
import { createLoadedToolHandlers } from "./loaded-tools.js";
...
export const TOOL_DEFINITIONS = [
    ...
    },
    {
        ...
        description: "Show which roll of film is currently loaded in which camera, plus which cameras are empty. A loaded roll is held out of trip reservations until it is unloaded.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    {
        name: "load_film",
        description: "Load a roll of film into a camera. The roll stays in inventory but is held: it cannot be reserved for a trip or loaded into another camera until it is unloaded. One roll per camera.",
        inputSchema: {
            type: "object",
            properties: {
                camera_id: {
                ...
                shot_at_iso: {
                    type: "number",
                    description: "Exposure index the roll is being shot at, e.g. 800 for ISO 400 film pushed a stop. Defaults to box speed.",
                },
                notes: {
                    type: "string",
    ...
    {
        name: "unload_film",
        description: "Finish a loaded roll. outcome 'shot' consumes one roll from inventory and logs it to usage history; outcome 'unused' releases the hold and leaves the count untouched.",
        inputSchema: {
            type: "object",
            properties: {
                loaded_id: {
                ...
                outcome: {
                    type: "string",
                    description: "'shot' if the roll was exposed, 'unused' if it came out of the camera unshot",
                    enum: ["shot", "unused"],
                },
                trip_id: {
                    type: "string",
                    description: "Optional trip to attribute the shot roll to, so it appears in that trip's usage history",
                },
            },
