import { addFilmToTripForUser, createTripForUser, deleteTripForUser, getFilmsWithAvailabilityForUser, getTripByIdForUser, getTripsForUser, removeFilmFromTripForUser, updateFilmQuantityInTripForUser, } from "../trips/service.js";
function jsonResult(data) {
    return {
        content: [{ type: "text", text: JSON.stringify(data, null, 2) }],
    };
}
export function createTripToolHandlers(supabase, userId) {
    ...
    async function getTripDetails(args) {
        const { trip_id } = args;
        if (!trip_id) {
      ...
      `)
        ...
        const filmRows = (tripFilms || []);
        const gearRows = (tripGear || []);
        const totalRolls = filmRows.reduce((sum, tf) => sum + tf.quantity, 0) || 0;
        const totalFilmValue = filmRows.reduce((sum, tf) => sum + tf.quantity * (tf.films?.price || 0), 0) || 0;
        const totalGearValue = gearRows.reduce((sum, tg) => sum + (tg.gear?.purchase_price || 0), 0) || 0;
        return jsonResult({
            trip,
            summary: {
    ...
    async function editTrip(args) {
        ...
        if (cleanedData.start_date && cleanedData.end_date) {
            if (new Date(String(cleanedData.end_date)) <
    ...
    async function reserveFilmForTrip(args) {
        ...
        const availableCount = film.available_count || 0;
        if (availableCount < quantity) {
            throw new Error(`Not enough available stock. Available: ${availableCount}, Requested: ${quantity}`);
        }
    ...
    async function removeFilmReservation(args) {
    ...
    async function updateFilmReservationQuantity(args) {
      ...
      `)
        ...
        const reservationData = existingReservation;
        const tripTitle = reservationData.trips[0]?.title || "";
        const filmName = reservationData.films[0]?.name || "";
        const filmBrand = reservationData.films[0]?.brand || "";
        if (userId) {
            await updateFilmQuantityInTripForUser(supabase, userId, trip_id, film_id, quantity);
        }
        else {
    ...
    async function getFilmsWithAvailability(args) {
        ...
        const totalValue = films?.reduce((sum, film) => sum + (film.price || 0) * (film.available_count || 0), 0) || 0;
        return jsonResult({
            summary: {
