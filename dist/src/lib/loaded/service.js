import { insertLoadedFilm, listActiveLoadedFilmsByUser, listCamerasByUser, listShotAtIsoHistoryByUser, markLoadedFilmUnloaded, revertLoadedFilmUnload, selectActiveLoadForCamera, selectCameraById, selectLoadedFilmById, } from "./repository.js";
import { listFilmsWithAvailabilityByUser } from "../trips/repository.js";
import { reduceFilmCountForUser } from "../films/service.js";
export async function getLoadedFilmsForUser(supabase, userId) {
 ...
 */
export async function getFilmsAvailableForLoadingForUser(supabase, userId) {
...
export async function getCameraLoadStatesForUser(supabase, userId) {
 ...
 */
export async function getEiPrefillsForUser(supabase, userId) {
...
export async function loadFilmIntoCameraForUser(supabase, userId, data) {
    const { data: camera, error: cameraError } = await selectCameraById(supabase, data.camera_id, userId);
    ...
    const { data: existing } = await selectActiveLoadForCamera(supabase, data.camera_id, userId);
    if (existing) {
        throw new Error(`${camera.brand} ${camera.name} already has a roll loaded. Finish that one first.`);
    }
    ...
    if (!film) {
        throw new Error("That film has no rolls available — it may be reserved for a trip or already loaded in another camera");
    }
...
export async function unloadFilmForUser(supabase, userId, loadedId, data) {
    const { data: entry, error: entryError } = await selectLoadedFilmById(supabase, loadedId, userId);
    if (entryError || !entry) {
    ...
    const { data: unloaded, error: unloadError } = await markLoadedFilmUnloaded(supabase, loadedId, userId, data.outcome);
    if (unloadError) {
        throw unloadError;
    }
    if (!unloaded) {
    ...
    if (data.outcome === "shot") {
        const result = await reduceFilmCountForUser(supabase, userId, loadedEntry.film_id, 1, buildUsageNote(loadedEntry), data.trip_id);
        if (result?.error) {
