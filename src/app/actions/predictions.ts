...
export async function getPredictiveAnalysis(): Promise<{
  ...
  try {
    ...
    const weeklyForecast = enhanceWeeklyPrediction(baseWeeklyForecast, weeklyTripImpact);
    const monthlyForecast = enhanceMonthlyPrediction(baseMonthlyForecast, monthlyTripImpact, monthlyTripCosts);
    ...
    const shootingInsights = generateShootingInsights(weeklyData, monthlyData, trendAnalysis, plannedTrips);
    
    // Stock recommendations with trip planning
    const stockRecommendations = generateStockRecommendations(allUsageData, weeklyForecast, monthlyForecast, plannedTrips);
  ...
  try {
    ...
    for (let month = 1; month <= 12; month++) {
      ...
      if (monthData && monthData.length > 0) {
        ...
        monthlyMultipliers[monthNames[month - 1]] = overallAverage > 0 ? monthAverage / overallAverage : 1.0;
      } else {
    ...
    return {
      data: null,
      error: error instanceof Error ? error.message : "Failed to calculate seasonal patterns"
    };
  }
}

// Helper functions
function enhanceWeeklyPrediction(basePrediction: WeeklyPrediction, tripImpact: number): WeeklyPrediction {
...
function enhanceMonthlyPrediction(basePrediction: MonthlyPrediction, tripImpact: number, tripCosts: number): MonthlyPrediction {
  if (tripImpact === 0) return basePrediction;
...
function generateShootingInsights(weeklyData: UsageData[], monthlyData: UsageData[], trend: TrendAnalysis, plannedTrips: PlannedTripImpact[]): string[] {
  ...
  if (trend.significance) {
    if (trend.direction === 'up' && trend.strength > 50) {
      insights.push(`📈 Strong upward trend: +${Math.abs(trend.changeRate).toFixed(1)}% per month`);
    } else if (trend.direction === 'down' && trend.strength > 50) {
      insights.push(`📉 Declining usage: ${Math.abs(trend.changeRate).toFixed(1)}% per month`);
    }
  }
  ...
  const historicalAvg = calculateMean(monthlyData.slice(0, -1).map(m => m.rolls_used));
  ...
  if (upcomingTrips.length > 0) {
    const totalTripRolls = upcomingTrips.reduce((sum, trip) => sum + trip.reservedRolls, 0);
    if (upcomingTrips.length === 1) {
      insights.push(`🎒 "${upcomingTrips[0].tripTitle}" planned in ${upcomingTrips[0].daysFromNow} days (${upcomingTrips[0].reservedRolls} rolls)`);
    } else {
      insights.push(`🗓️ ${upcomingTrips.length} trips planned this month (${totalTripRolls} rolls total)`);
    }
  }

  // Trip frequency insights
  if (plannedTrips.length > 2) {
    insights.push("📸 You're a frequent trip planner - great for consistent shooting!");
  }
...
function generateStockRecommendations(allUsageData: AllUsageData[], weeklyForecast: WeeklyPrediction, monthlyForecast: MonthlyPrediction, plannedTrips: PlannedTripImpact[]): StockRecommendation[] {
  ...
  const totalUsage = Object.values(typeUsage).reduce((sum, count) => sum + count, 0);
  ...
  Object.entries(typeUsage).forEach(([filmType, usage]) => {
    ...
    const plannedTripUsage = plannedTrips
      .filter(trip => trip.impactsMonth)
    ...
    if (totalPredictedUsage > 8) {
      action = 'buy';
      const tripPart = plannedTripUsage > 0 ? ` (${Math.round(plannedTripUsage)} from trips)` : '';
      reason = `High predicted usage: ~${Math.round(totalPredictedUsage)} rolls next month${tripPart}`;
      urgency = 'medium';
      ...
      reason = `Moderate usage predicted: ~${Math.round(totalPredictedUsage)} rolls${tripPart}`;
      urgency = 'low';
...
function generateBudgetAlert(monthlyData: UsageData[], monthlyForecast: MonthlyPrediction): BudgetAlert | undefined {
  ...
  if (predictedCost > avgCost * 1.5) {
    return {
      type: 'high',
      message: 'Next month\'s predicted lab costs are significantly higher than usual',
