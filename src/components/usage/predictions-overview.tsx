"use client";

import { useEffect, useState } from "react";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Alert, AlertDescription } from "@/components/ui/alert";
import { getPredictiveAnalysis, PredictiveAnalysis } from "@/app/actions/predictions";
import { PredictiveChart } from "./predictive-chart";
import { SeasonalPatterns } from "./seasonal-patterns";
import { TrendAnalysisComponent } from "./trend-analysis";
import { 
  TrendingUp, 
  TrendingDown, 
  Calendar, 
  DollarSign, 
  Film, 
  Target,
  AlertTriangle,
  Lightbulb,
  ShoppingCart
} from "lucide-react";

export function PredictionsOverview() {
  const [predictions, setPredictions] = useState<PredictiveAnalysis | null>(null);
  const [loading, setLoading] = useState(true);
  const [mounted, setMounted] = useState(false);

  useEffect(() => {
    setMounted(true);
  }, []);

  useEffect(() => {
    async function loadPredictions() {
      try {
        const result = await getPredictiveAnalysis();
        if (result.data) {
          setPredictions(result.data);
        }
      } catch (error) {
        console.error('Error loading predictions:', error);
      } finally {
        setLoading(false);
      }
    }

    if (mounted) {
      loadPredictions();
    }
  }, [mounted]);

  if (!mounted || loading) {
    return (
      <div className="space-y-4">
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {[...Array(4)].map((_, i) => (
            <Card key={i}>
              <CardHeader className="animate-pulse">
                <div className="h-4 bg-gray-200 rounded w-3/4"></div>
              </CardHeader>
              <CardContent className="animate-pulse">
                <div className="h-8 bg-gray-200 rounded w-1/2"></div>
              </CardContent>
            </Card>
          ))}
        </div>
        <div className="animate-pulse h-32 bg-gray-200 rounded"></div>
      </div>
    );
  }

  if (!predictions) {
    return (
      <Card>
        <CardContent className="text-center py-8">
          <p className="text-muted-foreground">No prediction data available</p>
          <p className="text-sm text-muted-foreground mt-2">
            Use more film to generate accurate forecasts
          </p>
        </CardContent>
      </Card>
    );
  }

  const getTrendIcon = (trend: string) => {
    switch (trend) {
      case 'increasing': return <TrendingUp className="h-4 w-4 text-green-600" />;
      case 'decreasing': return <TrendingDown className="h-4 w-4 text-red-600" />;
      default: return <Target className="h-4 w-4 text-blue-600" />;
    }
  };

  const getTrendColor = (trend: string) => {
    switch (trend) {
      case 'increasing': return 'text-green-600';
      case 'decreasing': return 'text-red-600';
      default: return 'text-blue-600';
    }
  };

  const getConfidenceColor = (confidence: number) => {
    if (confidence >= 80) return 'bg-green-100 text-green-800';
    if (confidence >= 60) return 'bg-yellow-100 text-yellow-800';
    return 'bg-orange-100 text-orange-800';
  };

  return (
    <div className="space-y-6">
      {/* Prediction Cards */}
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {/* Weekly Forecast */}
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">This Week&apos;s Forecast</CardTitle>
            <Calendar className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="flex items-center space-x-2">
              <div className="text-2xl font-bold">{predictions.weeklyForecast.expected}</div>
              <span className="text-sm text-muted-foreground">rolls</span>
              {getTrendIcon(predictions.weeklyForecast.trend)}
            </div>
            <div className="flex items-center justify-between mt-2">
              <p className="text-xs text-muted-foreground">
                {predictions.weeklyForecast.conservative}-{predictions.weeklyForecast.optimistic} range
              </p>
              <Badge variant="outline" className={getConfidenceColor(predictions.weeklyForecast.confidence)}>
                {predictions.weeklyForecast.confidence}%
              </Badge>
            </div>
          </CardContent>
        </Card>

        {/* Monthly Forecast */}
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Next Month Outlook</CardTitle>
            <Film className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="flex items-center space-x-2">
              <div className="text-2xl font-bold">{predictions.monthlyForecast.expected}</div>
              <span className="text-sm text-muted-foreground">rolls</span>
              {getTrendIcon(predictions.monthlyForecast.trend)}
            </div>
            <div className="flex items-center justify-between mt-2">
              <p className="text-xs text-muted-foreground">
                {predictions.monthlyForecast.conservative}-{predictions.monthlyForecast.optimistic} range
              </p>
              {predictions.monthlyForecast.peakPeriod && (
                <Badge variant="secondary" className="text-xs">
                  Peak Period
                </Badge>
              )}
            </div>
          </CardContent>
        </Card>

        {/* Budget Forecast */}
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Lab Budget</CardTitle>
            <DollarSign className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">
              €{predictions.monthlyForecast.developmentCost.expected.toFixed(0)}
            </div>
            <p className="text-xs text-muted-foreground">
              €{predictions.monthlyForecast.developmentCost.min.toFixed(0)}-€{predictions.monthlyForecast.developmentCost.max.toFixed(0)} range
            </p>
            <p className="text-xs text-muted-foreground mt-1">
              Next month development costs
            </p>
          </CardContent>
        </Card>

        {/* Trend Summary */}
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Overall Trend</CardTitle>
            {getTrendIcon(predictions.trendAnalysis.direction === 'up' ? 'increasing' : 
                          predictions.trendAnalysis.direction === 'down' ? 'decreasing' : 'stable')}
          </CardHeader>
          <CardContent>
            <div className={`text-2xl font-bold ${getTrendColor(
              predictions.trendAnalysis.direction === 'up' ? 'increasing' : 
              predictions.trendAnalysis.direction === 'down' ? 'decreasing' : 'stable'
            )}`}>
              {predictions.trendAnalysis.direction === 'up' ? 'Growing' : 
               predictions.trendAnalysis.direction === 'down' ? 'Declining' : 'Stable'}
            </div>
            <p className="text-xs text-muted-foreground">
              {Math.abs(predictions.trendAnalysis.changeRate).toFixed(1)}% per month
            </p>
            <Badge variant="outline" className="mt-1 text-xs">
              {predictions.trendAnalysis.significance ? 'Significant' : 'Mild trend'}
            </Badge>
          </CardContent>
        </Card>
      </div>

      {/* Budget Alert */}
      {predictions.budgetAlert && (
        <Alert>
          <AlertTriangle className="h-4 w-4" />
          <AlertDescription>
            <strong>Budget Alert:</strong> {predictions.budgetAlert.message} - 
            €{predictions.budgetAlert.amount.toFixed(2)} {predictions.budgetAlert.comparison}
          </AlertDescription>
        </Alert>
      )}

      {/* Planned Trips */}
      {predictions.plannedTrips.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Calendar className="h-5 w-5" />
              Upcoming Trips
            </CardTitle>
            <CardDescription>
              Planned trips with film reservations
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {predictions.plannedTrips.slice(0, 4).map((trip, index) => (
                <div key={index} className="flex items-center justify-between">
                  <div className="space-y-1">
                    <div className="text-sm font-medium">{trip.tripTitle}</div>
                    <div className="text-xs text-muted-foreground">
                      {new Date(trip.tripDate).toLocaleDateString()} • {trip.daysFromNow} days away
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <Badge variant="outline" className="text-xs">
                      {trip.reservedRolls} rolls
                    </Badge>
                    <Badge variant="secondary" className="text-xs">
                      €{trip.developmentCost}
                    </Badge>
                  </div>
                </div>
              ))}
              {predictions.plannedTrips.length > 4 && (
                <div className="text-xs text-muted-foreground text-center pt-2">
                  +{predictions.plannedTrips.length - 4} more trips planned
                </div>
              )}
            </div>
          </CardContent>
        </Card>
      )}

      {/* Predictive Charts */}
      <div className="grid gap-4 lg:grid-cols-2">
        <PredictiveChart type="weekly" forecastPeriods={4} />
        <PredictiveChart type="monthly" forecastPeriods={3} />
      </div>

      {/* Seasonal Patterns */}
      <SeasonalPatterns />

      {/* Trend Analysis */}
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <TrendingUp className="h-5 w-5" />
            Trend Analysis
          </CardTitle>
          <CardDescription>
            Deep insights into your shooting patterns and trends
          </CardDescription>
        </CardHeader>
        <CardContent>
          <TrendAnalysisComponent />
        </CardContent>
      </Card>

      {/* Insights and Recommendations */}
      <div className="grid gap-4 md:grid-cols-2">
        {/* Shooting Insights */}
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Lightbulb className="h-5 w-5" />
              Shooting Insights
            </CardTitle>
            <CardDescription>
              Patterns detected in your photography habits
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {predictions.weeklyForecast.patternInsights.map((insight, index) => (
                <div key={index} className="text-sm">
                  {insight}
                </div>
              ))}
              {predictions.shootingInsights.map((insight, index) => (
                <div key={index + 100} className="text-sm">
                  {insight}
                </div>
              ))}
            </div>
          </CardContent>
        </Card>

        {/* Stock Recommendations */}
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <ShoppingCart className="h-5 w-5" />
              Stock Recommendations
            </CardTitle>
            <CardDescription>
              Film purchasing suggestions based on your predicted usage
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {predictions.stockRecommendations.map((rec, index) => (
                <div key={index} className="flex items-center justify-between">
                  <div className="space-y-1">
                    <div className="text-sm font-medium">{rec.filmType}</div>
                    <div className="text-xs text-muted-foreground">{rec.reason}</div>
                  </div>
                  <div className="flex items-center gap-2">
                    {rec.recommendedQuantity && (
                      <Badge variant="outline" className="text-xs">
                        {rec.recommendedQuantity} rolls
                      </Badge>
                    )}
                    <Badge 
                      variant={rec.action === 'buy' ? 'default' : 'secondary'}
                      className={`text-xs ${
                        rec.urgency === 'high' ? 'bg-red-100 text-red-800' :
                        rec.urgency === 'medium' ? 'bg-yellow-100 text-yellow-800' :
                        'bg-green-100 text-green-800'
                      }`}
                    >
                      {rec.action === 'buy' ? `Buy (${rec.urgency})` : 'Sufficient'}
                    </Badge>
                  </div>
                </div>
              ))}
            </div>
          </CardContent>
        </Card>
      </div>
    </div>
  );
}