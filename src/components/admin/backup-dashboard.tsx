...
export function BackupDashboard({ stats, user }: BackupDashboardProps) {
  ...
  const handleTableBackup = async (tableName: string, format: 'json' | 'csv' = 'json') => {
    ...
    try {
      ...
      console.log(`✅ Table backup completed: ${tableName} (${data.length} records)`)
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <Card className="border-green-200 bg-green-50/50">
        <CardHeader>
          ...
          <CardDescription>
            Export all your data in a single JSON file. This includes all tables with complete relationships preserved.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-muted-foreground mb-2">
                This will download {stats.total.toLocaleString()} records across {tables.length} tables
              </p>
              {lastBackup && (
                <p className="text-xs text-muted-foreground">
      ...
      <Card className="border-blue-200 bg-blue-50/50">
        ...
        <CardContent className="text-sm space-y-2">
          <div>
            <strong>Security:</strong> All backups only include your personal data. No other users&apos; data is exported.
          </div>
          <div>
            <strong>Format:</strong> JSON files preserve relationships and are suitable for restoration. CSV files are spreadsheet-friendly.
          </div>
          <div>
            <strong>Storage:</strong> Files are generated locally and downloaded to your device. Nothing is stored on external servers.
          </div>
          <div>
            <strong>Data Included:</strong> All records including soft-deleted items for complete data preservation.
          </div>
        </CardContent>
      </Card>
    </div>
