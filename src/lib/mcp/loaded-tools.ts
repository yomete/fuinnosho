...
import { unloadOutcomeValues } from "@/lib/loaded/schema";
import {
...
function describeEntry(entry: LoadedFilmEntry) {
  return {
    loaded_id: entry.id,
    camera: entry.camera
...
export function createLoadedToolHandlers(
  ...
  async function getLoadedFilms(
    args: ToolArgumentsByName["get_loaded_films"]
  ...
  async function loadFilm(
    args: ToolArgumentsByName["load_film"]
  ): Promise<MCPToolResult> {
    const resolvedUserId = requireUser();
    ...
    return jsonResult({
      success: true,
      message: `${entry.film?.brand ?? ""} ${entry.film?.name ?? "Film"} loaded into ${
        entry.camera ? `${entry.camera.brand} ${entry.camera.name}` : "the camera"
      }. It is held out of trip reservations until you unload it.`,
      loaded: describeEntry(entry),
    });
  }

  async function unloadFilm(
    args: ToolArgumentsByName["unload_film"]
  ): Promise<MCPToolResult> {
    const resolvedUserId = requireUser();
    ...
    if (!unloadOutcomeValues.includes(outcome as UnloadOutcome)) {
      throw new Error(
        `outcome must be one of: ${unloadOutcomeValues.join(", ")}`
      );
    }

    const entry = await unloadFilmForUser(supabase, resolvedUserId, loaded_id, {
      outcome: outcome as UnloadOutcome,
      trip_id,
    });
    ...
    return jsonResult({
      ...
      message:
        outcome === "shot"
          ? `Marked as shot. One roll of ${entry.film?.brand ?? ""} ${
              entry.film?.name ?? "film"
            } was consumed and logged to your usage history.`
