...
export function createGearToolHandlers(
  supabase: SupabaseClient,
  userId: string
): GearToolHandlers {
  async function createGear(
    args: ToolArgumentsByName["create_gear"]
    ...
    const gear = userId
      ? await createGearForUser(supabase, userId, {
          name,
          brand,
          type: type as Gear["type"],
          model,
          serial_number,
          purchase_date,
          purchase_price,
          condition: condition as Gear["condition"],
          notes,
        })
      : (
          await supabase
  ...
  async function listGear(
    args: ToolArgumentsByName["list_gear"]
  ): Promise<MCPToolResult> {
  ...
  async function editGear(
    args: ToolArgumentsByName["edit_gear"]
  ): Promise<MCPToolResult> {
  ...
  async function deleteGear(
    args: ToolArgumentsByName["delete_gear"]
  ): Promise<MCPToolResult> {
    const { gear_id } = args;
    if (!gear_id) {
      throw new Error("gear_id is required");
  ...
  async function getGearStats(
    args: ToolArgumentsByName["get_gear_stats"]
  ): Promise<MCPToolResult> {
    const { group_by = "type" } = args;

    const { gear, stats } = userId
      ? await getGearStatsForUser(
          supabase,
          userId,
          group_by as keyof Gear
        )
      : { gear: [], stats: {} };
  ...
  async function reserveGearForTrip(
    args: ToolArgumentsByName["reserve_gear_for_trip"]
  ): Promise<MCPToolResult> {
  ...
  async function removeGearReservation(
    args: ToolArgumentsByName["remove_gear_reservation"]
  ): Promise<MCPToolResult> {
    const { trip_id, gear_id } = args;
